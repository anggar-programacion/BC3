...
def leer_registros_C(archivo):
    ...
    with open(archivo, 'rb') as f:
        for linea in f:
            if linea.startswith(b'~C'):  # Verificar si la línea comienza con "~C"
                ...
                # Filtrar solo los registros cuyo TIPO es "2" (Maquinaria)
                if tipo_codigo != "2":
                    continue
    ...
    conceptos_dir = os.path.join(os.path.dirname(archivo), nombre_archivo, "Maquinaria")
    ...
    with open(os.path.join(conceptos_dir, f"{nombre_archivo}_MAQUINARIA.TXT"), 'w', encoding='utf-8') as file:
    ...
    print(f"\nTotal de registros de Maquinaria: {contador_maquinaria}")
