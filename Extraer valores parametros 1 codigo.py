...
def leer_registros_P(archivo_bc3, codigo_especifico):
    ...
    with open(archivo_bc3, 'rb') as f:
        ...
        for linea in f:
            ...
            # Leer y procesar los parámetros
            if leer_parametros and linea_decodificada.startswith('\\'):
                parametros.append(linea_decodificada.strip('\\'))
    ...
    # Guardar los parámetros en un archivo individual en el directorio creado
    ...
    # Guardar los valores de los parámetros en un archivo individual
    nombre_archivo_parametros = os.path.join(parametros_dir, f"{codigo_especifico}_parametros.txt")
    with open(nombre_archivo_parametros, 'w', encoding='utf-8') as f:
        for parametro in parametros:
            campos_parametro = parametro.split('\\')
            f.write(f"Nombre del parámetro: {campos_parametro[0]}\n")
            for valor in campos_parametro[1:]:
