...
def leer_Redactor(archivo_bc3):
    Redactor = None  # Variable para almacenar el redactor del archivo
        ...
        with open(archivo_bc3, 'rb') as f:
            for linea in f:
                if linea.startswith(b'~V'):  # Verificar si la línea comienza con "~V"
                    ...
                    if len(campos) > 1:  # Asegurarse de que haya al menos 2 campos
                        Redactor = campos[1]
...
def main():
    ...
    if archivo_bc3:
        Redactor = leer_Redactor(archivo_bc3)
        if Redactor:
            print("El redactor de este archivo es:", Redactor)
        else:
            print("No se encontró información de Redactor en el archivo.")
    else:
